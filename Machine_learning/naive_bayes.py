...
# -*- coding: utf-8 -*-
"""
Created on Mon Nov  4 18:22:41 2019
...
langs = list(df)
letters = list(df.index) # wcześniej index_col = 0 więc literki to moje indeskowanie
...
df_norm = df/df.sum(0)
df_norm = df_norm.apply( lambda x: x+ 1e-100,axis = 1).apply(np.log,axis = 1)
...
l = [x.lower().replace(" ","").translate(str.maketrans('', '', string.punctuation))  for x in sentences]

x.lower()# zmiejszenie wszystkich liter
x.replace(" ","") # wyrzucenie wszystkich spacji
x.translate(str.maketrans('', '', string.punctuation) # wywala wszytskie znaki interpunkcyjne

#lista_l= [x.split() for x in l] # ciekawa kwestia
 

import string
def naive_bayes(sent, langs, df):

    df_log = df.apply( lambda x: x+ 1e-100,axis = 1).apply(np.log,axis = 1)

    # normalize the sentence: remove spaces and punctuations, take lower case
    sent = sent.lower().replace(" ","").translate(str.maketrans('', '', string.punctuation))
    
    
    log_probs = {i:0 for i in langs}
    for lang in langs: # jezyki
        for i in sent:
            log_probs[lang] = df_log.loc[i,lang] + log_probs[lang]
        log_probs[lang] = np.exp(log_probs[lang])
    
    p_d =sum(log_probs.values())
            
        # TODO compute language probabilitie and order from most to least probable
    d2 = {k : v/p_d for k,v in log_probs.items()}
    probs = dict(sorted(d2.items(), key= lambda y:y[1],reverse = True))
